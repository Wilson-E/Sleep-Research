...
class BayesianPersonalizer:
    ...
    def update_from_log(
        ...
        # Single-night observation noise: large sigma → conservative updating.
        # ~5-10 nights of consistent data needed to meaningfully shift a coefficient.
        obs_sigma = 15.0

        if caffeine_active:
            total_mg = sum(d.get("mg", 0) for d in log_entry.caffeine_doses)
            total_cups = total_mg / 95.0
            if total_cups > 0.25:
                current = profile["caffeine_duration_min_per_cup"].mu
                implied = current + (per_pathway_residual / total_cups) * 0.5
                implied = _clamp(implied, 0.0, 40.0)
        ...
        if alcohol_active and log_entry.alcohol_drinks > 0.25:
            ...
            implied = _clamp(implied, 0.0, 15.0)
        ...
        if light_active:
            ...
            implied = _clamp(implied, 5.0, 90.0)
        ...
        if meal_active:
            ...
            implied = _clamp(implied, 0.3, 3.0)
